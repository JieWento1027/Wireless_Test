...
const u8 TX_ADDRESS[TX_ADR_WIDTH]={0x11,0x12,0x13,0x14,0x15}; //发送地址
const u8 RX_ADDRESS[RX_ADR_WIDTH]={0x11,0x12,0x13,0x14,0x15}; //发送地址
...
{ 	
	...
	GPIO_ResetBits(GPIOB,GPIO_Pin_5|GPIO_Pin_6|GPIO_Pin_7);//PB5,6,7上拉					 
...
//txbuf:待发送数据首地址
//返回值:0，接收完成；其他，错误代码
u8 NRF24L01_RxPacket(u8 *rxbuf1,u8 *rxbuf2,u8 *rxbuf3)
{
	u8 sta;		    							   
	SPI1_SetSpeed(SPI_BaudRatePrescaler_8); //spi速度为9Mhz（24L01的最大SPI时钟为10Mhz）   
	sta=NRF24L01_Read_Reg(STATUS);  //读取状态寄存器的值    	 
	...
	if(sta&RX_OK)//接收到数据
	{
		NRF24L01_Read_Buf(RD_RX_PLOAD,rxbuf1,RX_PLOAD_WIDTH);//读取数据
		NRF24L01_Read_Buf(RD_RX_PLOAD,rxbuf2,RX_PLOAD_WIDTH);//读取数据
		NRF24L01_Read_Buf(RD_RX_PLOAD,rxbuf3,RX_PLOAD_WIDTH);//读取数据
